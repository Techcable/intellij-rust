...
//    fun `test generic associated type binding in trait bound 2`() = testExpr("""
//        trait Tr { type Item<A>; }
//
...
//    fun `test generic associated type binding in trait object 2`() = testExpr("""
//        trait Tr {
//            type Item<A>;
//            fn foo(&self) -> Self::Item<u8>;
//        }
...
//    fun `test aliased generic associated type binding in trait object 2`() = testExpr("""
//        trait Tr {
//            type Item<A>;
//            fn foo(&self) -> Self::Item<u8>;
//        }
...
//    fun `test generic associated type binding in 'impl Trait' 2`() = testExpr("""
//        trait Tr {
//            type Item<A>;
//            fn foo(&self) -> Self::Item<u8>;
//        }
...
//    """)
    ...
    fun `test struct field with generic associated type`() = testExpr("""
        struct Foo<V: Trait, U> {
            input: <V as Trait>::Item<U>,
        }
        trait Trait { type Item<U>; }
        struct S; struct X<U>;
        impl Trait for S {
            type Item<U> = X<U>;
        }
    ...
    fun `test tuple struct field with generic associated type`() = testExpr("""
        struct Foo<V: Trait, U>(<V as Trait>::Item<U>);
        trait Trait { type Item<U>; }
        struct S; struct X<U>;
        impl Trait for S {
            type Item<U> = X<U>;
